...
- [七-2、激光发射器 / VS1838B / WS2812B 接线](#七-2激光发射器--vs1838b--ws2812b-接线)
...
> VS1838B 内置 38 kHz 带通滤波器和解调电路，OUT 引脚正常为高电平，接收到匹配信号时拉低。

| VS1838B 引脚 | 方向 | 连接至 | 说明 |
|:-----------:|:----:|:------:|------|
| **VCC** | ← | D153C **3.3V** 输出 | 3.3V 供电（与 ESP32 GPIO 电平兼容） |
| **GND** | — | 公共 **GND** | 共地 |
| **OUT** | → | ESP32 **GPIO 12** | Active LOW，代码配置 INPUT_PULLUP |

```
VS1838B 封装引脚（正面朝向自己，凸面朝上）：
    ┌─────────┐
    │         │
    │         │
    └──┬──┬──┘      OUT  GND  VCC
          ↓    ↓    ↓
          1    2    3
  （从左到右：1=OUT，2=GND，3=VCC）
```

### WS2812B 可编程 LED 灯条接线

> WS2812B 每颜 LED 内置控制 IC，通过单线串行协议（800kHz）控制 RGB 颜色。**初始化全部绿色，命中一次就有一颜 LED 变红，R2 键重置为绿色。**
...
> 左摇杆控制运动方向；L2 键控制激光发射；R2 键重置 LED 灯条。
...
| **FastLED 库** | **≥ 3.6.0** | 工具 → 管理库 → 搜索 `FastLED` 安装（用于 WS2812B 控制） |
...
⑤ 烧录完成后，串口监视器（115200 baud）应显示：
     ...
     L2 short/long press = laser  R2 = reset LEDs
```
