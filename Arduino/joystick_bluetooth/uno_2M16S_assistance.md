...
左边缘   │  [VIN][GND]  ← 外部电源输入（7~12V DC）                                    │  右边缘
         ...
         │  [Pin0/RX] [Pin1/TX]  ← 运行时接 UNO Pin5(TX)/Pin4(RX)                    │
         │                                                                              │
         │  ┌─────────────────────────────────────────────────────────────────┐        │
...
> ⚠️ **注意**：经核对 `2M16S_final.jpg`，该板**没有 USB-B 接口，也没有 ISP Header 烧录接口**。2M16S 拥有独立的 ATmega328P MCU，**必须单独烧录**，不能通过叠插 UNO 的 USB-B 接口一并写入（UNO 的 USB-B 只能访问 UNO 自身的 MCU）。唯一可行的烧录方式为：使用 **USB-to-TTL 串口适配器**连接 2M16S 板上的 **UART Header（TX/RX/GND/5V）** 引脚，通过板载 optiboot Bootloader 上传固件。注意 UART Header **无 RST 引脚**，需手动按板上 RST 按钮触发复位。详见第三章 3.2 节。
